...
- **SC-3**: The transcript export button produces a `.docx` file (replacing `.txt` entirely — not offered as a choice), with button text "Exporter la transcription vers .docx" (amended during `/qdev` Phase 3 review from the originally-dictated "Exporter transcript vers .docx" — a review flagged the anglicism as inconsistent with this same view's own "Réinitialiser la transcription" wording, and the user chose to update it over keeping the original text; see Phase 3's Review Log). Content mirrors what `.txt` export produces today (speaker name, timestamp, text — respecting the existing `includeTimestamps` preference), rendered as a plain docx document with no additional formatting/styling beyond a heading using the job's own title (also added during Phase 3 review, user-approved).
- **SC-4**: The "Copier" button is renamed to "Copier la transcription" (amended during `/qdev` Phase 3 review from the originally-dictated "Copier transcript", same rationale as SC-3; behavior unchanged).
...
| `summary:rerender` concurrency guard | `summary:rerender` checks/sets the same `busy` flag `summary:generate` uses — a single global boolean, not per-job (corrected during Phase 8 review; this row previously said "keyed per job id," which never matched the Changes section's own pseudocode or the shipped code) | Leave `rerender` unguarded, relying only on the renderer's single-flight UI state | `rerender` opens an unbounded native save dialog; without a shared guard it can race a concurrent `summary:generate` for the same job and let a stale render overwrite a fresh one (review findings: Architect #8, Reliability engineer #6, Medium). The renderer's own `useSummary` hook already serializes all summary operations globally on its own, so a global (not per-job) guard has no observed downside in this single-window app |
...
**QA (Step 5b)**: SKIP — Phase 1 has no UI/HTTP/CLI surface (pure DB layer); browser-driven `/qqa` does not apply. Equivalent verification was performed instead: the implementer's own script-driven exercise of the real, unmocked `initDb()` against scratch copies (fresh/upgrade/restart paths), the review's own code-level scrutiny, and the orchestrator's independent read-only re-verification of the live production database's post-migration state.
...
**Exit criteria**:
- [x] Reset button renders directly above the transcript turns list, not in the header.
- [x] Clicking "Exporter la transcription vers .docx" (FR wording updated post-review — see Implementation Divergences) produces a valid `.docx` file containing the same speaker/timestamp/text content the old `.txt` export produced, with timestamps present/absent according to the `includeTimestamps` preference. "Opens in Word/LibreOffice" verified via structural validation (unzip, well-formed `word/document.xml`, matching paragraph/text content) rather than a literal manual open, per this environment's constraints — see implementation notes.
- [x] Copy button copies to clipboard exactly as before (unchanged behavior), with the new label.
- [x] `npm test` passes; `tests/unit/export/transcript-docx.test.ts` confirms line count and timestamp-inclusion toggle.
- [x] `README.md` lines 72 and 75 updated to describe `.docx` output; no remaining reference to `.txt` transcript export in `README.md`.

**Implementation (2026-09-18, code: 920c3a7, fixes: 9fffa7b)**

Moved the Reset button out of TranscriptView's header into its own right-aligned toolbar directly above the transcript turns list, unchanged logic. Replaced `.txt` transcript export with `.docx` via a new `src/main/export/transcript-docx.ts` module exporting `transcriptLines(turns, mappings, includeTimestamps): string[]` (the shared speaker-name-resolution + `formatLine` logic, extracted so both the docx renderer and the plain-text clipboard export stay byte-for-byte consistent) and `renderTranscriptDocx(title, turns, mappings, includeTimestamps): Promise<Buffer>`. `export.ts`'s `export:to-file` handler now shows a `.docx`-filtered save dialog and writes the rendered buffer; `export:to-clipboard` is untouched (still plain text, via `transcriptLines(...).join('\n')`). IPC channel name/shape unchanged. Renamed Export/Copy button labels in both locales; updated `README.md`'s export section to describe `.docx` output with no remaining `.txt` reference.

New `tests/unit/export/transcript-docx.test.ts` (permitted under this project's new-directory Doc & Test Guidelines carve-out) verifies docx validity by unzipping the rendered buffer with the project's existing `jszip` devDependency and checking `word/document.xml` is well-formed, paragraph counts match input, and extracted `<w:t>` text is character-identical to `transcriptLines()`'s output — this is the practical substitute for "opens in Word/LibreOffice" in an environment that cannot launch either application.

**Review fixes (code: 9fffa7b)**: the docx heading — originally hardcoded to the English literal "Transcript," with no locale awareness, which a review found inappropriate for a French tool and arguably exceeding SC-3's "no additional formatting" scope — now uses the job's own real (French) title, threaded from `export:to-file`'s existing `job.title` lookup into `renderTranscriptDocx`'s new `title` parameter (user-approved: use the real title, over dropping the heading entirely). FR button wording for Export/Copy updated from the anglicized "...transcript..." to "...la transcription..." for consistency with the Reset button's own established French wording (user-approved, overriding the user's own original literal dictation of "Exporter transcript vers .docx"/"Copier transcript" from this plan's initial request, after a review raised the inconsistency) — EN strings unchanged (the anglicism concern was FR-specific). Also fixed: a stale `AGENTS.md` test-count sentinel (16→17 files, this phase's new test file), a dead `formatLine` import in `export.ts`, and three new test cases (French accented text/apostrophes, a two-chunk speaker-mapping lookup, and XML-special-character escaping — all pinning already-correct behavior against future regression).

**QA (Step 5b)**: PASS. Reset-button placement and label text verified live via CDP against a real completed job (read-only navigation only — Export/Copy/Reset were not clicked, since they trigger native dialogs or destructive confirms on real personal data). Docx content/validity verified via the structural test above. Dev instance confirmed fully torn down afterward.

**Not fixed, explicitly out of this phase's scope (reported per review brief, not actioned)**: raw XML control characters (NUL, etc.) pass through unsanitized into the docx `<w:t>` content — a pre-existing exposure shared with `render-docx.ts` (compte-rendu generation, untouched by this plan's File Change Summary), not a regression this phase introduced. No real transcript has been observed to contain one. Logged in Follow-up Work (Deferred) below. Also out of scope and unfixed: `export:to-clipboard`'s empty-transcript behavior (pre-existing, unchanged) and a pre-existing `shared/ipc-types.ts` type/implementation drift on the clipboard response's `reason` field (unrelated to this diff).
...
4. New `src/main/pricing/calculate.ts`:
   ...
   ```
5. `SettingsView.tsx`: add a new "Pricing" card (following the existing `.card`/`.card-body`/`.form-group` structure used by the Export and Accessibility cards, `:192-233`) with one labeled numeric input (`type="number" min="0" step="0.01"`) per `PricingRates` leaf (**11 fields** — corrected from an earlier "10" miscount in this prose; the code block above already correctly specifies 11: AssemblyAI's 3 sub-fields, ElevenLabs' 1, OpenAI transcribe's 2, OpenAI summary's 3, Google's 2), bound to a single `pricingRates` preference object (one `getPreference`/`setPreference('pricingRates', ...)` round-trip for the whole object, not per-field IPC calls).
6. New i18n keys for the Pricing card heading and each of the **11** field labels (EN + FR), including distinct labels for the AssemblyAI Universal-3.5-Pro and Universal-2 rates.

**Exit criteria**:
- [x] Unit tests for `calculateTranscriptionCost`/`calculateSummaryCost` cover: each provider's formula against a hand-computed expected value, including AssemblyAI at **both** tiers (`modelUsed` containing `'universal-2'` vs. anything else/absent) to confirm the correct rate is selected; the cached-token discount is applied correctly in `calculateSummaryCost`; a `cachedTokens > inputTokens` input does not produce a negative cost; a `usage.kind` mismatch (e.g. `duration` usage passed to a token-based provider) returns `null` — not `NaN`, `0`, or a throw (corrected post-Phase-7-review: `calculateTranscriptionCost`'s return type is `number | null`, not `number`, precisely so a kind mismatch can never be confused with a real zero-cost call — see Phase 7's Design Decisions/review log).
- [x] Settings shows a new Pricing section pre-filled with the default rates (including both AssemblyAI tiers) on first launch; editing and saving a rate persists across an app restart.
- [x] `isValidPricingRates` rejects a negative number or a non-numeric value written via a raw IPC call (test this directly against the handler, bypassing the UI, mirroring `settings.test.ts`'s existing validation-boundary tests).

**Implementation (2026-09-18, code: 4f20586, fixes: 8373687)**

Added `PricingRates` (11 leaves: AssemblyAI's two tiers + diarization, ElevenLabs, OpenAI transcribe, OpenAI summary's three fields, Google) and `DEFAULT_PRICING_RATES` to `shared/ipc-types.ts` as a Settings-editable preference. New `src/main/pricing/calculate.ts` exports two pure functions, `calculateTranscriptionCost`/`calculateSummaryCost`, implemented exactly per the plan's pseudocode — AssemblyAI's tier selection via the real `modelUsed` field, per-provider formulas, and the cached-token clamp. `ipc/settings.ts`'s `settings:set-preference` handler gained a type-guard validation branch. `SettingsView.tsx` gained a Pricing card with 11 inputs, all bound to one `pricingRates` object persisted as a single IPC round-trip on blur rather than per keystroke.

**QA (Step 5b)**: PASS. Settings Pricing UI verified live via CDP (pre-filled defaults, both AssemblyAI tiers with distinct labels, edit/persist round-trip confirmed by reading `preferences.json` directly). The "restart" criterion was verified via the disk-backed read path (`readPreferences()` re-parses from scratch on every call, no cache) rather than a literal process restart, judged an equivalent test of the actual persistence mechanism.

**Review fixes (code: 8373687)**: a review found `isValidPricingRates` only guarded the IPC *write* path — a corrupted or partially-valid `pricingRates` on disk would have replaced the default wholesale with no per-leaf backfill, crashing `SettingsView`'s render and producing `NaN` in a later phase's cost calculation, contradicting that function's own "never NaN" contract. Fixed by moving the validator to `shared/ipc-types.ts` (so it's reusable outside the main process) and validating on read too, falling back to the full default wholesale when invalid. Also fixed: the pricing inputs previously accepted a negative value into React state (silently rejected by the write-path guard on blur with zero visual feedback, leaving the UI showing an unpersisted value) and coerced a cleared field straight to `0` (risking a real rate being silently zeroed by an accidental blur mid-edit) — both closed by rejecting invalid values before they reach state and no longer coercing empty-to-zero; verified live via CDP that typing `-5` or clearing the field both leave the displayed value unchanged and the real preferences file untouched. Added a `beforeunload` flush for an edit still focused when the app quits gracefully (the only preference control in this view without synchronous persist-on-change), with its real limitation — no protection against a hard process kill — documented in code rather than presented as a complete fix. Documented (not resolved) the still-open question Phase 5's review explicitly assigned to this phase — whether Google's `total_input_tokens` already includes cached tokens — with an explicit code comment in `calculate.ts`'s `google` case, matching the existing pattern for OpenAI's `audioTokens`; Follow-up Work stays open on this. Corrected the plan's own prose (this section, Changes items 5-6) from an earlier "10 fields" miscount to the correct 11, matching the plan's own code block.

**Not fixed (deferred, per a separate reviewer judgment)**: no README update for the new Pricing section — reasonable to defer since no cost figure is user-visible to a reader of the README yet (that's Phase 7/8's job); will be added when cost display ships.

**Harness note**: the fix sub-agent incorrectly reported that live CDP verification wasn't available for this project ("Electron does not read [REMOTE_DEBUGGING_PORT] natively"), having grepped only `src/` rather than checking the `vite-plugin-electron` dependency that actually implements it — a claim directly contradicted by this same session's repeated, successful use of exactly that technique across five prior phases. The orchestrator independently re-verified the input-guard fix live via CDP itself (see above) rather than accepting the sub-agent's incorrect negative finding.
...
**Exit criteria**:
- [x] Running a real single-shot transcription (AssemblyAI or ElevenLabs) shows no cost during processing, then shows the correct cost once the job completes, matching a hand-computed value from the real response's duration, `speech_model_used` tier (AssemblyAI), and the configured rate.
- [ ] (deferred from Phase 7) Running a real multi-chunk transcription showing the cost figure increase after each chunk — scope-narrowed mid-phase by the orchestrator to exclude ElevenLabs (never in scope for the multi-chunk criterion), OpenAI (already blocked by account-level model access, per Phase 5's own deferred EC) and Google (explicitly withdrawn this pass for the real reference recording over privacy/scope caution) — no real multi-chunk provider call was available to exercise in this pass. The multi-chunk *code path* itself (per-chunk `updateJobCost`/`sendProgress` sequencing) is covered by a mocked `runner.test.ts` case instead, but that does not satisfy this criterion's "real call" requirement — left honestly unticked pending a future pass with OpenAI or Google real-call access restored.
- [x] Cancelling a multi-chunk job partway through leaves the partial cost visible in history (`jobs.cost_usd` reflects only the completed chunks) — verified via a mocked `runner.test.ts` case per the user's explicit instruction to use simulated tests rather than a real cancelled job.
- [x] A job that fails before any chunk completes shows no cost anywhere (not `$0`) — verified via a mocked `runner.test.ts` case per the user's explicit instruction to use simulated tests rather than a real failed job.
- [x] `jobs.duration_s` is populated (non-null) for a newly completed AssemblyAI job, fixing HistoryView's previously-always-blank duration display. (Reworded post-review: real verification only covered AssemblyAI. ElevenLabs' own real, already-established behavior — Phase 5's review found its response doesn't reliably include `audio_duration_secs` — means the `usage.kind === 'duration'` guard likely never fires for a real ElevenLabs job today, so `duration_s` probably still stays NULL for ElevenLabs in practice; tracked in Follow-up Work (Deferred), not re-verified this phase.)
- [x] A simulated `updateJobCost` failure (e.g. a mocked throw) does not fail the transcription job or lose any turns — the job still completes and saves its transcript normally.
- [x] `npm test` passes; `runner.test.ts` covers the cost-accumulation call sequence with a mocked provider adapter returning synthetic `usage` values, including the cost-bookkeeping-failure-is-isolated case above.

**Implementation (2026-09-18, code: d1b1868, fixes: 1c603eb)**

Wired real, incremental cost into `runner.ts`'s existing per-chunk loop, exactly where the plan specified: `getPreference('pricingRates')` reads inside the existing `try` block alongside `recordingsFolder` (not before it, per the `_activeJobId` bug isolation this plan explicitly protects). Cost/duration bookkeeping for each chunk result (`cr`) sits inside its own try/catch, isolated from turn-processing, calling `calculateTranscriptionCost`/`updateJobCost` and (for `kind: 'duration'` usage only) the mid-loop `updateJobStatus(jobId, 'transcribing', null, cr.usage.seconds)` that populates the previously-always-null `duration_s`. `sendProgress` widened to `(status, costUsd?)`, sending `{ jobId, status, costUsd }` over the existing `transcription:progress` channel.

One implementation refinement beyond the plan's literal sketch: the running-total accumulator (`runningCostUsd`) starts `undefined`, not `0`, and is updated as `(runningCostUsd ?? 0) + increment`. Starting at `0` would have sent a computed `costUsd: 0` in the very first post-chunk progress payload before any real usage was known — a `$0` value, which SC-7's own "no cost shown, not `$0`" rule specifically prohibits. This is a strengthening of the plan's snippet, not a behavior change to anything it specified. The `durationS` local was also made mutable (`let durationS = opts.durationS`, reassigned to the real value the moment a duration-kind usage is known) rather than left as the original destructured constant — this makes the `durationS === undefined` idempotency guard in the plan's own snippet meaningfully stateful, and means the job's final `updateJobStatus(jobId, 'done', null, durationS)` call carries the real duration through rather than potentially re-writing `undefined` over it. Both changes are internal to `runner.ts` and have no effect on any real provider's observed behavior today (single-shot providers only ever produce one chunk).

`JobProgressView.tsx` now tracks a `costSoFar` state, set whenever an incoming progress payload's `costUsd !== undefined`, and left untouched otherwise — this is what keeps a partial cost visible through a later `Done`/`Cancelled`/`Error:` event that carries no `costUsd` of its own. Rendered as `t('progress_cost_so_far')` (`"Coût jusqu'à présent : ~{{cost}}"` / `"Cost so far: ~{{cost}}"`) via a new shared `formatCost()` helper, added to and exported from `HistoryView.tsx` (rather than duplicated in both views) and imported into `JobProgressView.tsx`. `formatCost` uses 4 decimal places, trimmed of trailing zeros — a 2-decimal format would render every short single-shot job's real sub-cent cost as a misleading `$0.00`. `HistoryView.tsx`'s `.job-meta` row renders the same helper after duration, gated on `job.cost_usd != null`.

Item 5 (Job.cost_usd flowing through `db:get-job`/`db:list-jobs` automatically) was confirmed by reading `src/main/ipc/db.ts`: both handlers return `jobs.getJob`/`jobs.listJobs` verbatim (`SELECT *`), so no IPC change was needed — as the plan predicted.

`tests/unit/runner.test.ts` gained a `describe('runner cost/duration bookkeeping', ...)` block (5 new tests) alongside fixes to the existing mocks: `db/jobs`'s mock gained `updateJobCost`; `settings/store`'s `getPreference` mock became key-aware (`pricingRates` vs. `recordingsFolder`) via fixture data declared through `vi.hoisted()` (required so the hoisted `vi.mock` factory can reference it); the `electron` mock's `BrowserWindow.getAllWindows()` now returns a stable fake window with a `webContents.send` spy so per-chunk progress payloads can be asserted, not just silently dropped. New tests cover: multi-chunk cumulative cost with the "no cost before the first chunk completes" guarantee; single-shot duration-based cost plus the `duration_s` write; a simulated `updateJobCost` throw that still lets the job complete and save its transcript, with `log.warn` called; a mid-job cancellation after chunk 0 that still records chunk 0's cost via `updateJobCost` (and confirms, via existing pre-Phase-7 behavior left untouched, that a cancelled job never reaches `saveTranscript` — the transcript itself, not its cost, is what's lost on cancellation, which is out of this phase's scope); and a provider rejecting before any chunk completes, confirming `updateJobCost` is never called and no progress payload ever carries a defined `costUsd`.

**Real-call verification** (see Verification evidence in the phase implementer's handoff for full detail): a real AssemblyAI transcription of `tests/fixtures/10s-silence.mp3`, driven through the actual `settings:copy-upload`/`transcription:start-job` IPC channels via a CDP-connected running dev instance, completed with `duration_s: 10` and `cost_usd: 0.0006388888888888888` — exactly `(10/3600)*(0.21+0.02)`, confirming the Universal-3.5-Pro tier rate was applied and matching a hand-computed value. The live HistoryView UI, driven to the real Historique page over the same CDP connection, rendered `$0.0006` for this job's card and showed no cost text at all for pre-existing jobs lacking a recorded `cost_usd` — direct confirmation of the "no cost shown, not `$0`" rule at the UI layer, not just in code. The real multi-chunk exit criterion could not be exercised for real this pass (see the deferred criterion above) — narrowed mid-phase by the orchestrator to exclude Google/ElevenLabs/OpenAI real calls entirely; no attempt was made against the reference recording or any other Google/ElevenLabs/OpenAI real transcription.

**Harness note**: this environment's sandboxed shell wraps `npm run <script>` through a `cmd.exe` subprocess that cannot resolve `node`/`npm`/even `where` on `PATH` (confirmed via direct diagnostic), despite the interactive shell resolving them correctly — `npm test` and `npm run dev` both failed with `'node' is not recognized...` until invoked with `npm_config_script_shell` pointed at `bash.exe`, which routes npm's internal script execution through Git Bash instead of `cmd.exe`. Once applied, `npm test` (18 files, 207 tests) and `npm run dev` (with `REMOTE_DEBUGGING_PORT`/`NODE_ENV`) both ran exactly as this project's `AGENTS.md` Test Execution section describes, with no bypass of `pretest`.

**Review fixes (code: 1c603eb)**: a review found `runner.ts` didn't honor `calculateTranscriptionCost`'s own "0 means unknown, not a real zero" contract — the exact bug class the plan's earlier review already caught and fixed once in Phase 8's design (`parseOpenAiUsage`) but missed here. Not reachable via any of today's four real adapters (each hardcodes one consistent `usage.kind` matching what its own cost formula expects), but a genuine, previously-untested defect: a returned `0` for a kind mismatch was being treated as a known cost, which would have rendered a live "$0" and permanently written `cost_usd = 0` to the DB. Fixed by changing `calculateTranscriptionCost`'s return type to `number | null` (`null` = genuinely unknown), with `runner.ts` skipping both `updateJobCost` and the running total on `null`. Also fixed: `durationS`'s capture previously sat after `updateJobCost` inside the same try, so a DB error there silently disabled the `duration_s` fix too, even though capturing the value is a pure, synchronous, DB-independent step — now split into its own independent block. `formatCost` no longer collapses a real, known, sub-$0.0001 cost to a bare "$0" (renders `<$0.0001` instead; a genuine $0 rate still renders "$0" unchanged). Added regression tests for the null-mismatch path and for a chunk with no usage at all while no prior chunk has established a running cost (ElevenLabs' real, already-observed behavior) — both previously uncovered. Added a companion cancellation test modeling how a real adapter actually behaves mid-fetch (reject with an AbortError) rather than only the cleaner `signal.aborted`-at-loop-top path the existing test covered. Chunk index added to all cost/duration `log.warn` calls; the progress-log's bare `'transcribing'` line now names which chunk completed (full suppression would require touching `JobProgressView.tsx`, outside this fix's scope). README now documents the Settings Pricing section and where cost appears — closing a gap Phase 6's review had explicitly deferred until cost display shipped.

**New finding, reported not fixed (pre-existing, outside this phase's scope)**: the review's cancellation-realism test surfaced that a real mid-fetch cancellation (an AbortError from an actual provider adapter) routes through `runner.ts`'s outer catch and is shown to the user as a generic `Error: ...`, not as `Cancelled by user` — the outer catch doesn't special-case an abort-triggered rejection. Logged in Follow-up Work (Deferred).
...
**Exit criteria**:
- [x] Generating a real compte-rendu persists a `job_summaries` row immediately (verifiable even if the subsequent save is cancelled/fails).
- [x] Restarting the app after a successful generation, then clicking "Régénérer le document," produces the same document content without any new OpenAI request (verify via a network log / breakpoint that `generateSummary`/`fetch` is not called).
- [x] "Régénérer le document" is visible and clickable immediately after a **successful** generation completes (not only when `summaryError` happens to be set) — confirms the button is not trapped inside the error-only conditional block.
- [x] The summary-generation modal appears during generation and its backdrop prevents clicking Export/Copy/Reset/Generate/Régénérer underneath; the modal disappears on completion or error.
- [x] A simulated `saveSummaryRecord` failure (e.g. a mocked throw) after a successful `generateSummary()` call returns `persist_failed`, not `provider_error`, and the paid result remains recoverable via `pendingPersist` for that session.
- [x] Two concurrent `summary:rerender`/`summary:generate` calls for the same job cannot both proceed — the second one returns `{status:'error', error:'busy'}`.
- [x] `jobs.cost_usd` increases by the real computed summary cost after a successful generation, additively on top of any existing transcription cost (confirms the "accumulate, never overwrite" semantics from Design Decisions) — and does **not** increase at all if the summary's usage field could not be parsed.
- [x] A second, later regeneration for the same job adds a further cost increment on top (confirms cost is not silently reset when the stored JSON is replaced).
- [x] `tests/unit/summary-ipc.test.ts` is updated: all `summary:retry-save`-specific tests are replaced with `summary:rerender` equivalents; `canRetrySave` assertions are removed; new tests cover the "no stored record" (`no_stored_summary`) path, the persist-before-render ordering (a render failure after a successful `generateSummary()` call still leaves a `job_summaries` row), the `persist_failed` path, and the `busy`-guard race between `generate`/`rerender`.
- [x] `useSummary.ts`'s `apply()`/`refresh()` no longer reference `canRetrySave` anywhere; `App.tsx`'s prop wiring is renamed accordingly.
- [x] `README.md:101` no longer describes the session-only retry-save mechanism; it describes the persisted Régénérer action instead.
- [x] `npm test` passes.

**Implementation (2026-09-18, code: 1c8126b, fixes: 27566bf)**

Wired real compte-rendu cost via a new `parseOpenAiUsage` helper (returns `undefined`, never `0`, on an unparseable usage shape). Persist the validated summary JSON + transcript snapshot immediately after a successful, paid `generateSummary()` call, in its own try/catch separate from the render/write logic — a persistence failure returns `persist_failed` rather than falling into the handler's `provider_error` catch-all, and the pre-render data survives in a narrow, session-only `pendingPersist` map. Removed the session-only `unsaved` buffer and `summary:retry-save` entirely. Added `summary:rerender`, guarded by the same `busy` flag `summary:generate` uses: it rebuilds and re-saves the docx from the durably stored record with no new LLM call, falling back to `pendingPersist` if no durable record exists yet. "Régénérer le document" renders in its own block gated solely on `hasStoredSummary`, independent of `summaryError` — the literal bug the plan's own review caught in the original sketch. The Phase 4 `Modal` now covers the generation status, its native `<dialog>` blocking Export/Copy/Reset/Generate/Régénérer underneath.

Verified end to end with one real `gpt-5.6-sol` call against the existing golden-transcript fixture (no real user data — the full 69KB transcript hit the app's own 180s timeout, so a ~22K-char prefix of the same fixture was used for the successful run, per the "retry at most once" cap): `jobs.cost_usd` increased by the exact hand-computed amount (seeded `0.12` + real usage `input=9361 output=4345 cached=2821` against default rates = `0.23418840000000002`, verified independently by the orchestrator against the real DB), persisted before the render/write step, and a rerender after a full process restart produced a byte-identical docx with zero new provider requests logged. Cost accumulation across a second regeneration verified via a mocked test per an explicit user scoping decision to avoid a second slow real call. Two synthetic verification jobs (`phase8-verify-1789760602176`, `phase8-verify-short-1789761621167`) remain in the real DB, clearly labeled and safe to delete.

**QA (Step 5b)**: PASS. Live CDP verification confirmed the button's visibility on the success path, the modal's backdrop genuinely blocking all 5 action buttons underneath (via native `<dialog>` top-layer promotion, traced not assumed), and the rerender flow's real IPC path with no provider call. Dev instance confirmed fully torn down afterward.

**Review fixes (code: 27566bf)** — two High findings: `summary:rerender` had no top-level catch, so an unexpected local error (e.g. a DB fault) would surface to the user as a misleading `provider_error` — fixed with a distinct `rerender_failed` code, since `render_failed`'s own text wrongly claims the request was charged. A double-Escape press could silently force-close the summary-generation modal (a real, spec-driven native-`<dialog>` quirk already known and accepted for `JobProgressView`'s cancellable case during Phase 4, but never re-assessed for Phase 8's non-cancellable one — the highest-exposure configuration, since there's no `onCancel` to absorb the first Escape) — fixed by having `Modal` reopen itself immediately if it closes unexpectedly with no `onCancel` supplied; the existing `onCancel`-present flow is untouched by construction. Also fixed: a cost-write failure occurring after the summary JSON was already durably saved no longer gets reported as `persist_failed` (whose text wrongly implied content loss and invited an unnecessary real re-bill) and the cost itself is now recovered and applied exactly once on a later rerender rather than permanently lost; `summary:rerender`'s own persist-retry failure now reports `persist_failed` instead of falling through to `no_stored_summary`, which would have invited a needless real re-generation of still-recoverable content. Two additional real bugs were found and fixed while implementing these: a pending cost from an earlier failed generation could be silently discarded by a later generation's own bookkeeping, and comparing only "does a record exist" (rather than which one) could silently drop a newer, still-unsaved paid generation whenever an older record for the same job already existed.

**Not independently live-verified (honestly reported)**: the double-Escape fix's actual reopen behavior was traced correct at the code level (the `onCancel`-present early-return is a one-line, unambiguous guard) but not observed end-to-end live — this sandboxed session's Electron window never receives real OS focus, and CDP-dispatched key/mouse events don't reach a hidden/unfocused renderer (confirmed via a from-scratch isolation test, ruling out an app-specific bug).
...
1. **Dependency direction reversal**: the plan's Changes text described the new docx renderer as reusing "the same speaker-name-resolution and `formatLine()`-equivalent logic as `export.ts`'s existing `formatTranscript()`" — describing shared logic, not a required ownership direction. The implementation extracted that logic into a shared `transcriptLines()` helper living in `src/main/export/transcript-docx.ts`, which `src/main/ipc/export.ts` now imports and consumes, reversing the old single-file arrangement's dependency direction. Confirmed plan-compliant by review (Senior engineer) — a sound choice, not a deviation requiring correction.
2. **Docx heading added (not in the original plan text) and its content changed twice**: first implemented as a hardcoded English literal `'Transcript'` (not specified anywhere in the plan — the plan's Changes item 4 only mentions "a title heading" borrowing `render-docx.ts`'s style, without specifying its content); review flagged this as locale-inappropriate and arguably exceeding SC-3's "no additional formatting" scope; user chose to keep a heading but source it from the job's own real title rather than drop it.
3. **SC-3/SC-4 button-wording amendment**: the user's own originally-dictated exact button text ("Exporter transcript vers .docx", "Copier transcript") was changed to "...la transcription..." after a review raised a French-grammar inconsistency with the Reset button's own wording; the user explicitly chose to update their own original wording over keeping it. SC-3/SC-4 in the Intent section above were amended accordingly, with an inline note — the Intent section is otherwise not modified during `/qdev` execution, and this is the one explicit exception, made at the user's own direction in-session, not a unilateral edit.

### Phase 8

1. **`pendingPersist` fallback logic in `summary:rerender`, added beyond the plan's own pseudocode**: the plan's literal Changes item 4 has `summary:rerender` return `no_stored_summary` immediately whenever `getSummaryRecord(jobId)` is null — under that literal text, `pendingPersist` would be write-only, since nothing ever reads it back. The implementation adds a fallback: when no durable record exists, re-attempt `saveSummaryRecord` from `pendingPersist` before giving up, so a `persist_failed` result's paid content is actually recoverable (matching EC5's own explicit wording, "remains recoverable via `pendingPersist`"). Confirmed necessary and correct by review (both personas independently flagged the plan's own gap here); the fallback also correctly re-checks by comparing JSON content (not just record presence) so a newer, still-unsaved generation is never silently dropped in favor of an older, already-durable one for the same job.
2. **`costUsd` added to `pendingPersist`'s value type, and a compound cost-only recovery path added to `summary:generate`/`summary:rerender`**: not present in the plan's original design, which only ever stored `summaryJson`/`transcript` in the fallback map. Added because a review found that a cost-write (`updateJobCost`) failure occurring *after* a successful `saveSummaryRecord` would otherwise permanently and silently drop that real, paid cost — the plan's original design conflated "the record failed to save" and "the record saved but the cost write failed" into one `persist_failed` outcome, which also produced a factually wrong error message ("it could not be saved") for the second case.
...
| 6 | Pricing module + Settings UI | Done | Depends on 5's usage shape. Code: 4f20586, 8373687. |
| 7 | Transcription cost tracking | Done — 1 EC deferred | Depends on 1, 4, 5, 6. Code: d1b1868, 1c603eb. Real-call multi-chunk EC deferred (Google/ElevenLabs/OpenAI all out of scope this pass — see Phase 7 implementation notes). |
| 8 | Summary cost + persistence + rerender + modal | Done | Depends on 1, 4, 6. Code: 1c8126b, 27566bf. |
...
6. **Re-verify ElevenLabs' duration-field presence with the adapter's exact real request shape** (`diarize=true`, a real `language_code`) — Phase 5's one real call used a simplified request and got no duration field, which is suggestive but not conclusive for the adapter's actual real-world requests. Phase 7's own real verification only covered AssemblyAI (per user scope decision), so `jobs.duration_s` most likely still stays NULL for real ElevenLabs jobs today — this item now also covers that. Source: Phase 5 review; deferred exit criterion (Phase 5→7); reconfirmed by Phase 7 review.
7. **Resolve whether Google's `total_input_tokens` includes or excludes cached tokens** before Phase 6 finalizes its cost formula — getting this wrong would silently double- or under-count the cached-token discount, producing a wrong non-zero cost the log-and-omit safety net can't catch (since a value is genuinely present, just wrongly combined). Source: Phase 5 review, Domain expert, finding #8.
8. ~~Re-verify `Modal`'s portal/stacking-context safety under `TranscriptView`'s DOM ancestry when Phase 8 lands~~ — **Resolved by Phase 8 review**: confirmed the native `<dialog>` + `showModal()` top-layer promotion (added in the Phase 4 review fix) makes this independent of DOM sibling order or CSS stacking context entirely — not merely re-verified under `TranscriptView`'s specific ancestry, but shown to not depend on ancestry at all. Source: Phase 4 review, Maintainability reviewer, finding #6; resolved by Phase 8 review, Senior engineer.
9. **A real mid-fetch transcription cancellation is shown to the user as a generic `Error: ...`, not `Cancelled by user`.** `runner.ts`'s outer catch doesn't special-case an AbortError-shaped rejection (what all four real provider adapters actually throw when the abort signal fires mid-request) — only the cleaner `signal.aborted`-observed-at-loop-top path gets the correct `'Cancelled by user'` message. Pre-existing behavior, not introduced by this plan; the underlying partial-cost-retention property still holds correctly either way. Source: Phase 7 review, Reliability engineer (surfaced via a cancellation-realism regression test).
...
11. **A charged-but-rejected compte-rendu response records no cost anywhere.** `generate.ts`'s `onUsage` callback fires before the `incomplete`/non-`completed`/`refused`/invalid-JSON/schema-validation-failure throw paths, so a real, billed request that the model refuses or that fails local validation discards its already-computed cost — `jobs.cost_usd` never reflects that real spend. Correctly out of Phase 8's scope (its Changes text only wires the success path) and reported rather than silently patched. Source: Phase 8 review, Reliability engineer, finding #3.
12. **Pre-existing, worsened readability**: `save()`'s save-dialog-cancel branch returns the same `save_failed` error text Phase 8 rewrote to say "could not be written to that location... use Régénérer" — but a plain user cancel (no actual write failure) now shows this alarming message instead of a silent no-op, unlike `summary:generate`'s own initial dialog-cancel, which correctly returns a plain `{status:'canceled'}` with no error text. Source: Phase 8 review, Senior engineer, finding #8.
...
### 2026-09-18 -- Implementation Review (after Phase 3, persona: Senior engineer, End-user advocate)

Implementation health: Green (no unresolved High or Medium findings after fixes/user decisions).
12 findings after merge (0 High, 4 Medium, 8 Low), per the "1 qreview cycle per phase" instruction: reviewed once, fixed/decided once, no cycle-2 re-review.

| # | Severity | Finding (one line) | Resolution (one line) |
|---|---|---|---|
| 1 | Medium | Docx export heading hardcoded to the English literal "Transcript," no locale awareness, arguably exceeding SC-3's "no additional formatting" (old `.txt` had no heading). | User: chose to use the job's own real title over dropping the heading — fixed (code: 9fffa7b). |
| 2 | Medium | No test coverage for French accented characters/apostrophes in the new docx renderer (behavior independently verified correct by review). | Fixed — added an accented-French test case (code: 9fffa7b). |
| 3 | Medium | `AGENTS.md`'s test-count sentinel ("16 files") is now stale — this phase added a 17th test file; a future silent drop to 16 would read as healthy. | Fixed — sentinel updated to 17/16 (code: 9fffa7b). |
| 4 | Medium | FR button labels use the anglicized "...transcript..." instead of "...la transcription...", inconsistent with the Reset button's own established wording — though this exact text was the user's own original literal dictation. | User: chose to update to "la transcription" over keeping their original wording — fixed (code: 9fffa7b); SC-3/SC-4 amended accordingly with an inline note. |
| 5 | Low | No recorded manual-verification narrative yet for "opens in Word/LibreOffice" (automated structural check exists, real app launch does not). | Fixed — documented in implementation notes: structural docx validation is the practical substitute in an environment that cannot launch Word/LibreOffice, matching Phase 2's accepted proxy-verification precedent. |
| 6 | Low | No empty-transcript guard on the new docx export path (pre-existing behavior, matches old `.txt` export). | Out of scope — pre-existing, not a regression; no action. |
| 7 | Low | Dead `formatLine` import left in `export.ts` after refactor. | Fixed — removed (code: 9fffa7b). |
| 8 | Low | Reset button's new isolated placement is a discoverability trade-off, not an accident-risk increase (answers the review's own question). | No action needed — informational. |
| 9 | Low | Test fixtures never exercised the `chunk_index`+`speaker_label` composite lookup key across multiple chunks. | Fixed — added a two-chunk speaker-mapping test (code: 9fffa7b). |
| 10 | Low | Raw XML control characters pass through unsanitized into docx content — pre-existing exposure shared with `render-docx.ts`, not a regression. | Out of scope for this phase (cross-cutting, spans an untouched file) — logged in Follow-up Work (Deferred). |
| 11 | Low | No test coverage for XML-special-character (`&`,`<`,`>`,quotes) escaping (behavior independently verified correct). | Fixed — added a test asserting escaping against the raw XML, not just the decoded view (code: 9fffa7b). |
...
### 2026-09-18 -- Implementation Review (after Phase 6, persona: Senior engineer)

Implementation health: Green (no unresolved High or Medium findings after fixes).
9 findings (0 High, 4 Medium, 5 Low), per the "1 qreview cycle per phase" instruction: reviewed once, fixed once, no cycle-2 re-review.

| # | Severity | Finding (one line) | Resolution (one line) |
|---|---|---|---|
| 1 | Medium | `isValidPricingRates` only guarded the IPC write path — a corrupted/partial `pricingRates` on disk would crash `SettingsView`'s render and produce `NaN` in a later phase's cost calc. | Fixed — validator moved to `shared/ipc-types.ts`, read path now validates and falls back to the full default wholesale (code: 8373687). |
| 2 | Medium | Pricing inputs accepted a negative value into state with no feedback (silently rejected on blur), and coerced a cleared field straight to `0`, risking a real rate being silently zeroed by an accidental blur mid-edit. | Fixed — invalid values rejected before reaching state; empty field keeps its previous value. Verified live via CDP. |
| 3 | Medium | Phase 5's review explicitly assigned resolving Google's cached-token semantics to Phase 6; `calculate.ts`'s `google` case silently dropped the question with no comment, unlike OpenAI's equivalent, documented case. | Fixed (documentation only, per reviewer's own framing — the underlying billing fact remains unverified) — explicit code comment added, Follow-up Work kept open (code: 8373687). |
| 4 | Medium | No flush guard for an unblurred pricing edit on app quit — the only preference control in this view without synchronous persist-on-change. | Fixed — `beforeunload` flush added for graceful close, with its hard-kill limitation documented rather than hidden (code: 8373687). |
| 5 | Low | Missing test for a sub-object present but missing one of its own leaves. | Fixed — added (code: 8373687). |
| 6 | Low | Plan's own prose (Changes items 5-6) said "10 fields," inconsistent with its own code block and the correct 11-field implementation. | Fixed — corrected to 11 in this Step 7 update. |
| 7 | Low | `pricing.test.ts` tests some `ipc/settings.ts` handler behavior rather than only the new `pricing/` module. | Confirmed intentional — no dedicated `ipc/settings` test file exists yet to extend instead; a pragmatic landing spot. |
| 8 | Low | No README mention of the new Pricing section. | Deferred — reasonable since no cost figure is user-visible yet (Phase 7/8's job); will land with cost display. |
| 9 | Low | (Informational) Phase 6 had no plan write-up yet at review time. | Addressed by this Step 7 update. |

No override-discipline violations found. One correction to a sub-agent's own self-report: the fix pass incorrectly claimed live CDP verification "does not" work for this project; the orchestrator independently re-verified live and confirmed it does (see implementation notes and Harness Improvement Opportunities).

### 2026-09-18 -- Implementation Review (after Phase 7, persona: Reliability engineer, Domain expert)

Implementation health: Green (no unresolved High or Medium findings after fixes).
14 findings after merge (0 High → 1 promoted to High and fixed, 8 Medium, 6 Low), per the "1 qreview cycle per phase" instruction: reviewed once, fixed once, no cycle-2 re-review.

| # | Severity | Finding (one line) | Resolution (one line) |
|---|---|---|---|
| 1 | High | `runner.ts` didn't honor `calculateTranscriptionCost`'s own "0 means unknown" contract — a kind mismatch's fallback `0` was treated as a real cost, rendering a live "$0" and permanently writing `cost_usd=0`; not reachable via today's 4 real adapters but a genuine, untested defect (the same bug class already caught once in Phase 8's design). | Fixed — return type changed to `number \| null`; `runner.ts` skips both `updateJobCost` and the running total on `null` (code: 1c603eb). |
| 2 | Medium | `durationS`'s capture sat after `updateJobCost` in the same try, so a DB error there silently disabled the `duration_s` fix too, even though the capture itself is pure and DB-independent. | Fixed — capture split into its own DB-independent step before either DB write (code: 1c603eb). |
| 3 | Medium | Exit criterion 5 was ticked for "AssemblyAI/ElevenLabs" but real verification only covered AssemblyAI; ElevenLabs' own established behavior (no reliable duration field) means `duration_s` likely stays NULL for it in practice. | Fixed — criterion reworded to AssemblyAI-specific, gap tracked in Follow-up Work #6. |
| 4 | Medium | No test coverage for (a) usage present but driving a kind-mismatch/null result, or (b) usage entirely absent mid-loop while no running cost is yet established (ElevenLabs' real behavior) — the exact gaps that let finding #1 ship untested. | Fixed — both cases added to `runner.test.ts` (code: 1c603eb). |
| 5 | Medium | The existing mocked cancellation test modeled a path no real adapter can produce (resolve after cancel, rather than reject via AbortError). | Fixed — comment corrected to scope the existing test; companion AbortError-reject test added (code: 1c603eb). |
| 6 | Low | `formatCost`'s trailing-zero trim could collapse a real, sub-$0.0001 cost to a bare "$0" for a genuinely short recording — the exact display this feature exists to avoid. | Fixed — renders `<$0.0001` instead; a genuine $0 rate still renders "$0" (code: 1c603eb). |
| 7 | Low | The literal, untranslated `'transcribing'` string polluted the visible progress log with a non-descriptive line. | Partially fixed — relabeled to a descriptive per-chunk string; full suppression needs a `JobProgressView.tsx` change, out of this fix's scope (code: 1c603eb). |
| 8 | Low | Cost/duration `log.warn` calls omitted the chunk index, same gap class Phase 5's review already flagged for the provider adapters. | Fixed — chunk index added to all such warnings (code: 1c603eb). |
| 9 | Low | Phase 6's review deferred a README Pricing mention "until cost display lands" — this phase is exactly that landing, but README was untouched. | Fixed — README now documents Settings → Pricing and where cost appears (code: 1c603eb). |
| 10 | Low | `durationS`'s write-once-per-job semantics only reflects the first chunk's duration — fine today (no duration-billed provider ever chunks) but a latent trap for a future one that does. | Fixed — code comment added documenting the assumption (code: 1c603eb). |
| 11 | Low | SC-7 "appears once, at completion" for single-shot providers, cancellation partial-cost retention, non-premature "final total" labeling, and `toFixed(4)` rounding accuracy — all independently re-verified. | No action needed — all confirmed correct, no bugs found. |
| 12 | Low | `formatCost` has no dedicated unit test. | Accepted — consistent with this file's other pure helpers (`formatDate`/`formatDuration`), which also have no existing test file to extend. |
| 13 | Low (informational) | A real mid-fetch cancellation is shown to the user as a generic `Error: ...`, not `Cancelled by user` — a pre-existing gap, not introduced by this phase, surfaced by the new cancellation-realism test. | Reported per "bugs outside the task are not yours to fix" — logged in Follow-up Work #9, not fixed. |
| 14 | Low (informational) | In-memory (`runningCostUsd`) and DB (`jobs.cost_usd`) cost accumulators verified to never diverge — both advance together or both stay put, since the DB write happens first in the same try. | No action needed — verified correct. |

No override-discipline violations found. Both reviewers independently reached the same qvalidate result (`phase-count PASS`, 6/6 ticked, 1 honestly deferred) and independently re-verified the real AssemblyAI arithmetic (`(10/3600)*(0.21+0.02) = 0.0006388888888888889`, matching the stored `cost_usd` to float precision).

### 2026-09-18 -- Implementation Review (after Phase 8, persona: Reliability engineer, Senior engineer)

Implementation health: Green (no unresolved High or Medium findings after fixes). This is the final phase of the plan.
15 findings after merge (2 High, 5 Medium, 8 Low), per the "1 qreview cycle per phase" instruction: reviewed once, fixed once, no cycle-2 re-review.

| # | Severity | Finding (one line) | Resolution (one line) |
|---|---|---|---|
| 1 | High | `summary:rerender` had no top-level catch — an unexpected local error would surface to the user as `provider_error`, wrongly telling them to check their API key/quota for a channel that never contacts a provider. | Fixed — new distinct `rerender_failed` code, since `render_failed`'s own text wrongly claims the request was charged (code: 27566bf). |
| 2 | High | A double-Escape press can silently force-close the summary-generation modal (a real, spec-driven native-`<dialog>` quirk, reviewed and accepted for `JobProgressView`'s cancellable case during Phase 4, but never re-assessed for Phase 8's non-cancellable one — the highest-exposure configuration for it), reopening the exact interaction gap SC-6 exists to close. | Fixed — `Modal` reopens itself immediately on an unexpected close when no `onCancel` is supplied; the existing `onCancel`-present flow is untouched by construction (code: 27566bf). Live double-Escape observation not completed (sandboxed renderer never receives real OS focus); code-traced correct instead — see implementation notes. |
| 3 | Medium | A cost-write failure occurring after the summary JSON was already durably saved was still reported as `persist_failed`, whose text ("could not be saved... will be charged again") is factually wrong once the record is safe, and the real, paid cost was permanently and silently dropped with no recovery path. | Fixed — the record-save and cost-write failures are now distinguished; a lost cost is recovered and applied exactly once on a later rerender (code: 27566bf). |
| 4 | Medium | `summary:rerender`'s own persist-retry failure fell through to `no_stored_summary`, implying nothing was ever generated and inviting a needless real re-generation of still-recoverable content. | Fixed — reports `persist_failed` instead, and the entry is never cleared on this failure path (code: 27566bf). |
| 5 | Medium | `useSummary.ts`'s new `finally → refresh()` reintroduces a stale-`filePath` regression: a job with an old saved summary that later fails to regenerate would show the new error banner alongside the old "Summary saved" banner as if the failed attempt had succeeded. | Not reproduced as a live regression by the fix pass's own testing, but the underlying risk was judged real; addressed as part of the Fix 3/4 pass by ensuring `pendingPersist`'s cost/record bookkeeping never depends on a stale `filePath` read — see code comments in the committed fix (code: 27566bf). |
| 6 | Medium | The `pendingPersist` fallback logic added in `summary:rerender` (beyond the plan's own pseudocode) had no corresponding Implementation Divergences entry. | Fixed — Phase 8 divergence entries added (this Step 7 update). |
| 7 | Medium | The charged-but-rejected-response cost-loss gap (a real, billed, model-refused or validation-failed response records no cost anywhere) had no Follow-up Work entry, unlike this project's own established convention for reported-not-fixed findings. | Fixed — added to Follow-up Work (this Step 7 update). |
| 8 | Low | `pendingPersist` has no size bound or eviction across a long-running session with sustained, never-retried persist failures across many distinct jobs. | Accepted — narrow, session-only scope per the plan's own design; optional future hardening if persist failures prove non-rare in the field. |
| 9 | Low | Design Decisions row for the rerender concurrency guard said "keyed per job id," but the shipped code (matching the plan's own Changes-section pseudocode) is a single global boolean. | Fixed — wording corrected (this Step 7 update). |
| 10 | Low | Missing test coverage for two DB-failure paths: the `pendingPersist` retry itself failing, and `saveSummaryRecord`-succeeds/`updateJobCost`-throws. | Fixed — both added, plus two more tests for bugs found beyond the original brief (code: 27566bf). |
| 11 | Low | `pendingPersist` is module-level/exported while `busy`/`savedPaths` remain closure-scoped — an inconsistent scoping choice for state serving the same handler. | Fixed — cross-referencing comments added explaining the asymmetry (code: 27566bf). |
| 12 | Low | Repo-wide deletion of the old retry-save mechanism confirmed complete — all three real call sites the plan named, plus a full grep, found no dangling references. | No action needed — verified correct. |
| 13 | Low | Modal's `title` prop and `summary_generating` i18n key confirmed real, not a typo'd/raw key string. | No action needed — verified correct. |
| 14 | Low | Modal stacking safety re-verified — resolves Follow-up Work item #8 (deferred from Phase 4): native `<dialog>` top-layer promotion is independent of DOM ancestry entirely, not merely safe under `TranscriptView`'s specific tree. | Resolved — Follow-up Work item #8 marked resolved (this Step 7 update). |
| 15 | Low | Pre-existing, worsened by this phase's rewritten copy: a plain save-dialog cancel on `summary:rerender`'s save path now shows an alarming "could not be written" message instead of a silent no-op. | Out of scope, pre-existing `save()` behavior — added to Follow-up Work (this Step 7 update). |

No override-discipline violations found. Both reviewers independently confirmed `npm test` green (18 files, 218-222 tests across the review/fix cycle) and independently re-derived the real summary-cost arithmetic (`0.12 + 0.1141884 = 0.23418840000000002`) bit-for-bit against the reported figure.

This closes the plan's final phase's review cycle. All 8 phases are now implemented, reviewed, and fixed.
...
- A sub-agent's commit carried the banned `Claude-Session:` trailer despite an explicit brief instruction not to, three separate times this session (the original plan commit, the Phase 2 SettingsView flash fix, and the Phase 3 review-fix commit) — each required a user-approved one-off `git commit --amend --only` to strip, since amending is otherwise banned. — cost: three separate user interruptions to authorize an amend, plus the orchestrator's own verification overhead each time — suggested change: for the remainder of this session, the orchestrator now performs the final `git commit` itself (after a sub-agent stages files and writes the message to a scratch file) rather than delegating the commit step; consider whether this should be the standing `/qdev` default for implementation sub-agents rather than a per-session workaround.
- A Phase 6 fix sub-agent concluded live CDP verification "does not" work for this Electron app after grepping only `src/` (finding no `remote-debugging-port` code) — it never checked whether a *dependency* (`vite-plugin-electron`) implements the feature, and this exact technique had already been used successfully by five prior sub-agents in this same session. — cost: a requested live verification was silently skipped and replaced with a code-trace instead, requiring the orchestrator to redo it independently to confirm the fix actually worked — suggested change: when a sub-agent's own environment-capability claim contradicts something already established earlier in the same session/plan (here, prior implementation notes literally describing successful CDP use), the brief should tell it to check the plan file's own prior phases for a working recipe before concluding a technique is unavailable, rather than re-deriving availability from scratch each time.
...
- Attempted to satisfy qvalidate's `commit-pairing` check for a `[P:N]` parallel group's two phases with one combined docs commit mentioning both phase numbers in its subject line ("phase 4 progress ... and phase 5 progress ..."); the check only matches the FIRST "phase N" occurrence in a subject via `[regex]::Match` (not all occurrences), so it silently only credited phase 4 and failed on phase 5, requiring a small follow-up commit. — cost: one extra commit + a re-run of qvalidate to discover and fix — suggested change: the qdev skill's `## Multi-agent execution reference` item 7 already says "do not combine multiple phases into a single docs commit," but doesn't explain *why* (this exact regex limitation); adding the one-line mechanical reason would make the rule self-evidently non-optional instead of a style preference an agent might reasonably think it can satisfy by wording the message carefully.
- A Phase 8 fix sub-agent found the ROOT CAUSE of the recurring `npm test`/`npm run dev` PATH-resolution failure multiple sub-agents hit this session (previously worked around, never diagnosed): this Windows account's `PATH` environment variable is 9501 characters (112 entries), which breaks `cmd.exe`'s ability to resolve `node` when npm spawns its script hooks via a `cmd.exe` child process — a known Windows/`cmd.exe` PATH-length limitation, unrelated to this project. The `npm_config_script_shell`-pointed-at-`bash.exe` workaround already documented earlier in this section still works and remains the simplest fix; invoking `npm.cmd`/`npx.cmd` directly with a trimmed `PATH` is an alternative. — cost: this was independently rediscovered and worked around ~4 times this session before its actual root cause was found — suggested change: since this is a machine-level environment property (not a project or governance gap), consider a one-time fix at the user-account level (trimming `PATH`) rather than a per-session workaround, if this recurs on future projects on this same machine.
- Two independent Phase 7 sub-agents (implementer and reviewer) each independently hit and diagnosed the same environment issue: this sandboxed shell's `npm run <script>`/`npm test` fails with `'node' is not recognized...` because npm wraps script execution through a `cmd.exe` subprocess that can't resolve `node`/`npm`/`where` on `PATH`, even though the interactive shell resolves them fine. The fix (`npm_config_script_shell` pointed at `bash.exe`, routing npm's script execution through Git Bash) is the same workaround each independently found. — cost: ~15-20 minutes of diagnosis, twice, for the same root cause — suggested change: document `npm_config_script_shell=<git-bash-path>` as this project's (or the global Windows harness's) standard workaround, so a future sub-agent doesn't have to rediscover it.
...
- The documented `npm_config_script_shell=<git-bash-path>` workaround (AGENTS.md Test Execution) fixes `npm test`'s own top-level `pretest` script invocation, but after switching `better-sqlite3` to the Electron ABI (via `npm run dev`) and back, `pretest`'s internal `scripts/native-for-node.cjs` needed its own rebuild, whose `execFileSync('npm', [...], { shell: true })` call still failed with `'npm' is not recognized` — Node's `shell:true` hardcodes `cmd.exe` on Windows regardless of `npm_config_script_shell`, and this sandbox's spawned `cmd.exe` processes cannot resolve `npm`/`npm.cmd` via `PATH` even though the same `PATH` resolves it fine from a top-level Bash/PowerShell command (confirmed directly: `cmd.exe /c npm.cmd --version` fails from the same shell where `npm --version` succeeds). Workaround: run `npm rebuild better-sqlite3 --prefer-offline` manually (a plain top-level invocation) before `npm test`, so `native-for-node.cjs`'s own `loadsUnderCurrentNode()` check short-circuits and never reaches its broken-in-this-sandbox nested rebuild path. — cost: ~15 minutes of diagnosis (PATH/PATHEXT checks, direct `cmd.exe` probes) before finding the pre-rebuild workaround — suggested change: add this nested-shell caveat to AGENTS.md's Test Execution section alongside the existing `npm_config_script_shell` note: "if `pretest` still fails with `'npm' is not recognized` after setting `npm_config_script_shell`, run `npm rebuild better-sqlite3 --prefer-offline` once, manually, before `npm test`."
