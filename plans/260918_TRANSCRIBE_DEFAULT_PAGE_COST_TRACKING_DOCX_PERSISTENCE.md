...
| 6 | Low | The only real-world evidence Phase 1's migration path works end-to-end was the unplanned incident, which ran against the live file, not a copy as EC2 literally specifies. | User: accepted — the implementer's own EC2 verification script was already copy-based (seeded from the incident's genuine pre-migration backup); the live run was a separate, unintended event, not the recorded verification method. |
